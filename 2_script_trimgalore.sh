...
#copy new FastQC reports to laptop to view them
...
#note that illumina adapters (5’-3’) have the same first 13bp at both 5’ end (left of insert) and 3’end (right of insert), and TrimGalore use the first 13bp of the adapter to trim 
#quality cutoff is 20 (Phred score) by default (trim the 3’ end)
##(cutadapt does only 3’ quality trimming) 
#--fastqc   Run FastQC once trimming is complete
#--length 30 (default pair-cutoff=20 bp) both reads of a read-pair need to be longer than “length = bp” to be printed out as validated paired-end 
#--stringency 3  !By default set to 1 # set it at 3 (Cutadapt default)
#--trim-n Removes Ns from either side of the read


#same for the rest of samples